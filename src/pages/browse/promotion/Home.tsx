        ...
        <TopNavBack title={`View Promotion`} alignment="start" navigation={props.navigation} pops={1} />
            <ScrollView>
                ...
                <Layout style={[MainStyles.column_container, { paddingTop: 0, paddingStart: 30, paddingEnd: 30, backgroundColor: '#fff'}]}>
					...
					<IconText status="basic" title={promotion.promo_price} iconname="pricetags-outline" width={18} fontsize={14} fontweight="600" opacity={0.7} style={{ fontSize: 16, fontWeight: '600', lineHeight: 23, color: '#220622', opacity: 0.7, width: '100%' }} />
					<Text style={[MainStyles.title_a14, { width: '100%', marginTop: 20 }]}>Sale Price</Text>
					<IconText status="basic" title={promotion.sale_item_op} iconname="pricetags-outline" width={18} fontsize={14} fontweight="600" opacity={0.7} style={{ fontSize: 16, fontWeight: '600', lineHeight: 23, color: '#220622', opacity: 0.7, width: '100%' }} />
					<Text style={[MainStyles.title_a14, { width: '100%', marginTop: 20 }]}>Marked Down Price</Text>
					<IconText  status="basic"title={promotion.sale_item_mp} iconname="pricetags-outline" width={18} fontsize={14} fontweight="600" opacity={0.7} style={{ fontSize: 16, fontWeight: '600', lineHeight: 23, color: '#220622', opacity: 0.7, width: '100%' }} />
					<Text style={[MainStyles.title_a14, { width: '100%', marginTop: 20 }]}>Promotion Ends</Text>
					<IconText status="basic" title={promotion.end_date} iconname="calendar" width={18} fontsize={14} fontweight="600" opacity={0.7} style={{ fontSize: 14, fontWeight: '600', lineHeight: 23, color: '#220622', opacity: 0.7, width: '100%' }} />
					<Text style={[MainStyles.title_a16, { width: '100%', marginTop: 20 }]}>Location</Text>
					<Text status="basic" style={{ fontSize: 14, fontWeight: '600', lineHeight: 23, color: '#220622', opacity: 0.8, width: '100%' }}>{`${promotion.loc_add_one ? promotion.loc_add_one : '-'}\n${promotion.loc_add_two ? promotion.loc_add_two : '-'}\n${promotion.loc_city ? promotion.loc_city : '-'}\n${promotion.loc_province ? promotion.loc_province : '-'}\n${promotion.loc_zip_code ? promotion.loc_zip_code : '-'}`}</Text>
